...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(3, 16, 7, 1, 0, bias=False)
        self.conv2 = nn.Conv2d(16, 32, 3, 2, 0, bias=False)
        self.conv3 = nn.Conv2d(32, 64, 3, 2, 0, bias=False)
        self.conv4 = nn.Conv2d(64, 128, 3, 2, 0, bias=False)
        self.conv5 = nn.Conv2d(128, 256, 3, 2, 0, bias=False)
        self.conv6 = nn.Conv2d(256, 512, 3, 2, 0, bias=False)
        self.conv7 = nn.Conv2d(512, 1024, 3, 2, 0, bias=False)
    ...
    def encoder(self, x):
        # input is 3 x 218 x 178
        x = F.leaky_relu(self.bn_16(self.conv1(self.ref_pad3(x))), 0.2)
        # state size 16 x 218 x 178
        x = F.leaky_relu(self.bn_32(self.conv2(self.ref_pad1(x))), 0.2)
        # state size 32 x 109 x 89
        x = F.leaky_relu(self.bn_64(self.conv3(self.ref_pad1(x))), 0.2)
        # state size 64 x 55 x 45
        x = F.leaky_relu(self.bn_128(self.conv4(self.ref_pad1(x))), 0.2)
        # state size 128 x 27 x 23
        x = F.leaky_relu(self.bn_256(self.conv5(self.ref_pad1(x))), 0.2)
        # state size 256 x 14 x 12
        x = F.leaky_relu(self.bn_512(self.conv6(self.ref_pad1(x))), 0.2)
        # state size 512 x 7 x 6
        x = F.leaky_relu(self.bn_1024(self.conv7(self.ref_pad1(x))), 0.2)
        # state size 1024 x 4 x 3

        # Series of 4 residual blocks.
        for _ in range(self.n_res_blk):
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.conv_trans5 = nn.ConvTranspose2d(64, 32, 4, 2, 1, bias=False)
        self.conv_trans6 = nn.ConvTranspose2d(32, 16, 4, 2, 1, bias=False)

        self.conv = nn.Conv2d(16, 3, 7, 1, 0, bias=False)
    ...
    def decoder(self, x):
        ...
        x = x[:,:,0:55,0:45]
        # state size 64 x 55 x 45
        x = F.leaky_relu(self.bn_32(self.conv_trans5(x)), 0.2)
        # state size 32 x 110 x 90
        x = x[:,:,0:109,0:89]
        # state size 32 x 109 x 89
        x = F.leaky_relu(self.bn_16(self.conv_trans6(x)), 0.2)
        # state size 16 x 218 x 178
        x = torch.tanh(self.bn_3(self.conv(self.ref_pad3(x))))
        # state size 3 x 218 x 178
        return x
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.conv_trans1 = nn.ConvTranspose2d(512, 256, 4, 2, 1, bias=False)
        self.conv_trans2 = nn.ConvTranspose2d(256, 128, 4, 2, 1, bias=False)
        self.conv_trans3 = nn.ConvTranspose2d(128, 64, 4, 2, 1, bias=False)
        self.conv_trans4 = nn.ConvTranspose2d(64, 32, 4, 2, 1, bias=False)

        self.conv = nn.Conv2d(32, 3, 7, 1, 0, bias=False)

        self.n_res_blk = 6
        self.res_conv = nn.Conv2d(512, 512, 3, 1, 0, bias=False)
    ...
    def residual_block(self, ip):
        # input is 512 x 28 x 23
        x = F.leaky_relu(self.bn_512(self.res_conv(self.ref_pad1(ip))), 0.2)
        x = F.leaky_relu(self.bn_512(self.res_conv(self.ref_pad1(x))), 0.2)
        # output is 512 x 28 x 23
        return torch.add(ip, x)
        
    def forward(self, x):
        # input is 3 x 109 x 89
        x = F.leaky_relu(self.bn_64(self.conv1(self.ref_pad3(x))), 0.2)
        # state size 64 x 109 x 89
        x = F.leaky_relu(self.bn_128(self.conv2(self.ref_pad1(x))), 0.2)
        # state size 128 x 55 x 45
        x = F.leaky_relu(self.bn_256(self.conv3(self.ref_pad1(x))), 0.2)
        # state size 256 x 28 x 23
        x = F.leaky_relu(self.bn_512(self.conv4(self.ref_pad1(x))), 0.2)
        # state size 512 x 14 x 12

        # Series of 6 residual blocks.
        for _ in range(self.n_res_blk):
            x = self.residual_block(x)
            # state size 512 x 14 x 12

        x = F.leaky_relu(self.bn_256(self.conv_trans1(x)), 0.2)
        # state size 256 x 28 x 24
        x = x[:,:,:,0:23]
        # state size 256 x 28 x 23
        x = F.leaky_relu(self.bn_128(self.conv_trans2(x)), 0.2)
        # state size 128 x 56 x 46
        x = x[:,:,0:55,0:45]
        # state size 128 x 55 x 45
        x = F.leaky_relu(self.bn_64(self.conv_trans3(x)), 0.2)
        # state size 64 x 110 x 90
        x = x[:,:,0:109,0:89]
        # state size 64 x 109 x 89
        x = F.leaky_relu(self.bn_32(self.conv_trans4(x)), 0.2)
        # state size 32 x 218 x 178
        x = torch.tanh(self.bn_3(self.conv(self.ref_pad3(x))))
        # state size 3 x 218 x 178
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.conv6 = nn.Conv2d(1024, 1024, 4, 1, 1, bias=False)
        self.conv7 = nn.Conv2d(1024, 1, 4, 1, 1, bias=False)
    ...
    def forward(self, x):
        ...
        # state size. 128 x 54 x 44
        x = F.leaky_relu(self.bn_256(self.conv3(x)), 0.2)
        # state size. 256 x 27 x 22
        x = F.leaky_relu(self.bn_512(self.conv4(x)), 0.2)
        # state size. 512 x 13 x 11
        x = F.leaky_relu(self.bn_1024(self.conv5(x)), 0.2)
        # state size. 1024 x 6 x 5
        x = F.leaky_relu(self.bn_1024(self.conv6(x)), 0.2)
        # state size. 1024 x 5 x 4
