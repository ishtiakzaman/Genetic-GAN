...
# Trials from train_low was like this:
...
#optimizerGeneratorFemaleMid = optim.Adam(GeneratorFemaleMid.parameters(), lr=lr, betas=(beta1, 0.999))
#optimizerGeneratorMaleMid = optim.Adam(GeneratorMaleMid.parameters(), lr=lr, betas=(beta1, 0.999))
...
log_file = open('log/log_low_mid_' + str(trial) + '.txt', 'w')
graph_file = open('graph/graph_low_mid_' + str(trial) + '.csv', 'w')
graph_file.write('iters,kl,rec,cyc,real,fake,G,real_mid,fake_mid,G_mid\n')
epoch = 1
child_index = 0
...
while True:
    ...
    _, disc_f_in_img_mid = disc_f_data.load_batch('train')
    _, disc_m_in_img_mid = disc_m_data.load_batch('train')

    if f_in_img_mid.shape[0] != b_size or m_in_img_mid.shape[0] != b_size:
        continue
    if disc_f_in_img_mid.shape[0] != b_size or disc_m_in_img_mid.shape[0] != b_size:
        continue
    ...
    if iters % 50 == 0:
        print_str = '[trial: %d][iter: %d][epoch: %d/%d]\nL_KL: %.3f/%.3f\tL_Rec: %.3f/%.3f\tL_Cyc: %.3f/%.3f\nL_Real_D: %.3f/%.3f\tL_Fake_D: %.3f/%.3f\tL_Adv_G: %.3f/%.3f\nmid\tL_Real_D: %.3f/%.3f\tL_Fake_D: %.3f/%.3f\tL_Adv_G: %.3f/%.3f' % (trial, iters, epoch, num_epochs, loss_kl_female.item(), loss_kl_male.item(), loss_rec_female.item(), loss_rec_male.item(), loss_cyc_female.item(), loss_cyc_male.item(), loss_real_D_female.item(), loss_real_D_male.item(), loss_fake_D_female.item(), loss_fake_D_male.item(), loss_G_female.item(), loss_G_male.item(), loss_real_D_female_mid.item(), loss_real_D_male_mid.item(), loss_fake_D_female_mid.item(), loss_fake_D_male_mid.item(), loss_G_female_mid.item(), loss_G_male_mid.item())
        print(print_str)
        log_file.write(print_str + '\n')
        log_file.flush()
        graph_str = str(iters) + ',' + str((loss_kl_female.item() + loss_kl_male.item()) * 0.5) + ',' + str((loss_rec_female.item() + loss_rec_male.item()) * 0.5) + ',' + str((loss_cyc_female.item() + loss_cyc_male.item()) * 0.5) + ',' + str((loss_real_D_female.item() + loss_real_D_male.item()) * 0.5) + ',' + str((loss_fake_D_female.item() + loss_fake_D_male.item()) * 0.5) + ',' + str((loss_G_female.item() + loss_G_male.item()) * 0.5) + ',' + str((loss_real_D_female_mid.item() + loss_real_D_male_mid.item()) * 0.5) + ',' + str((loss_fake_D_female_mid.item() + loss_fake_D_male_mid.item()) * 0.5) + ',' + str((loss_G_female_mid.item() + loss_G_male_mid.item()) * 0.5)
    ...
    if epoch_happened == True:
        ...
        n_col_low = 10
        n_col_mid = 12
        f_in_img_mid = postprocess_convert_images(f_in_img_mid.cpu())
        m_in_img_mid = postprocess_convert_images(m_in_img_mid.cpu())
        ...
        for ci in xrange(5):
            ...
            offspring_f_out_img = DecoderFemale(z_offspring)
            offspring_m_out_img = DecoderMale(z_offspring)
            offspring_female_list_low.append(postprocess_convert_images(offspring_f_out_img.detach().cpu()))
            offspring_male_list_low.append(postprocess_convert_images(offspring_m_out_img.detach().cpu()))
            offspring_female_list_mid.append(postprocess_convert_images(GeneratorFemaleMid(offspring_f_out_img).detach().cpu()))
            offspring_male_list_mid.append(postprocess_convert_images(GeneratorMaleMid(offspring_m_out_img).detach().cpu()))
        img_row_low = 55
        img_col_low = 45
        img_row_mid = 109
        img_col_mid = 89
        ...
        for r in xrange(n_row):
            image[r*img_row_mid:(r+1)*img_row_mid, img_col_mid*0:img_col_mid*1, :] = f_in_img_mid[r]
            image[r*img_row_mid:(r+1)*img_row_mid, img_col_mid*1:img_col_mid*2, :] = m_in_img_mid[r]
            for ci in xrange(5):
        ...
        imsave('sample_low_mid/train_' + str(trial) + "_" + str(epoch) + '.jpg', image)    
        ...
        torch.save(GeneratorFemaleMid.state_dict(), 'weight/generator_mid_f_' + str(trial) + '_' + str(epoch) + '.pt')
        torch.save(GeneratorMaleMid.state_dict(), 'weight/generator_mid_m_' + str(trial) + '_' + str(epoch) + '.pt')
        torch.save(DiscriminatorFemaleMid.state_dict(), 'weight/discriminator_mid_f_' + str(trial) + '_' + str(epoch) + '.pt')
        torch.save(DiscriminatorMaleMid.state_dict(), 'weight/discriminator_mid_m_' + str(trial) + '_' + str(epoch) + '.pt')
        epoch += 1
        if epoch > num_epochs:
            break
